...
public class restaurant_payment_receipt extends AppCompatActivity  implements NavigationView.OnNavigationItemSelectedListener{
    private String session_name,shop_name,un,table,date_time,order_id,p_type;;
    private double discount_rate,change,sub_total,grand_total,cash;
    private int discount;
    private TextView date_time_invoice_retail,sub_total_invoice_retail,discount_invoice_retail,grand_total_invoice_retail,cash_invoice_retail,change_invoice_retail,cash_payment_method,change_pid,payment_receipt_shop_name,table_invoice,oid;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_restaurant_payment_receipt);
        Intent i = getIntent();
        ...
        table_invoice= (TextView)findViewById(R.id.table_invoice);
        table_invoice.setText("Table : "+table);
        ...
        if(p_type.equals("Cash Paid")){
            ...
            DatabaseReference db = FirebaseDatabase.getInstance().getReference("transaction").child(session_name).child("Restaurant").child(date_time).child("pid");
            db.setValue(PID);
        }
        ...
        if(from_transaction){
            ...
            btn_new_sale_retail.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    Intent myIntent = new Intent(restaurant_payment_receipt.this, restaurant_take_order.class);
        ...
        db = FirebaseDatabase.getInstance().getReference("bridge_entity").child(session_name).child("Restaurant").child(date_time);
        db.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if(dataSnapshot.hasChildren()){
                    ...
                    adapter_orderList = new MyAdapter_OrderList(restaurant_payment_receipt.this,list){
                        @Override
        ...
        btn_email_retail.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (ContextCompat.checkSelfPermission(restaurant_payment_receipt.this, Manifest.permission.READ_EXTERNAL_STORAGE) == PackageManager.PERMISSION_DENIED){
                    ActivityCompat.requestPermissions(restaurant_payment_receipt.this, new String[] {Manifest.permission.READ_EXTERNAL_STORAGE}, 1);
                }
                if (ContextCompat.checkSelfPermission(restaurant_payment_receipt.this, Manifest.permission.WRITE_EXTERNAL_STORAGE) == PackageManager.PERMISSION_DENIED){
                    ActivityCompat.requestPermissions(restaurant_payment_receipt.this, new String[] {Manifest.permission.WRITE_EXTERNAL_STORAGE}, 1);
                }

                if (ContextCompat.checkSelfPermission(restaurant_payment_receipt.this, Manifest.permission.MOUNT_UNMOUNT_FILESYSTEMS) == PackageManager.PERMISSION_DENIED){
                    ActivityCompat.requestPermissions(restaurant_payment_receipt.this, new String[] {Manifest.permission.MOUNT_UNMOUNT_FILESYSTEMS}, 1);
                }
                getRecipient();
            }
        });
        ...
        nav_head.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent myIntent = new Intent(restaurant_payment_receipt.this, profile.class);
                myIntent.putExtra("Session ID",session_name);
                myIntent.putExtra("Shop",shop_name);
                myIntent.putExtra("UN",un);
                startActivity(myIntent);
    ...
    private void getRecipient() {
        LayoutInflater li = LayoutInflater.from(restaurant_payment_receipt.this);
        View promptsView = li.inflate(R.layout.promt_email, null);
        AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(restaurant_payment_receipt.this);
    ...
    protected void sendEmail(){
        ...
        Uri p = FileProvider.getUriForFile(restaurant_payment_receipt.this, restaurant_payment_receipt.this.getApplicationContext().getPackageName() + ".provider", filelocation);
        emailIntent .putExtra(Intent.EXTRA_STREAM,  p);
        try {
            startActivity(Intent.createChooser(emailIntent, "Send mail..."));
            Log.i("Finish sending email...", "");
        } catch (android.content.ActivityNotFoundException ex) {
            Toast.makeText(restaurant_payment_receipt.this, "There is no email client installed.", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    public boolean onNavigationItemSelected(MenuItem item) {
        ...
        if (id == R.id.nav_take_order) {
            ...
            Intent myIntent = new Intent(restaurant_payment_receipt.this, restaurant_menu.class);
            myIntent.putExtra("Session ID",session_name);
            myIntent.putExtra("Shop",shop_name);
            myIntent.putExtra("UN",un);
            startActivity(myIntent);
        } else if (id == R.id.nav_dashboard_2) {
            Intent myIntent = new Intent(restaurant_payment_receipt.this, restaurant_dashboard.class);
            myIntent.putExtra("Session ID",session_name);
            myIntent.putExtra("Shop",shop_name);
            myIntent.putExtra("UN",un);
            startActivity(myIntent);
        } else if (id == R.id.nav_retail ){
            Intent myIntent = new Intent(restaurant_payment_receipt.this, retail_main_menu.class);
            myIntent.putExtra("Session ID",session_name);
            myIntent.putExtra("Shop",shop_name);
            myIntent.putExtra("UN",un);
            startActivity(myIntent);
